...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        help='Input file containing the paths ' 'of configs to be trained/tested.',
    )
    parser.add_argument(
        '--output_file',
        required=True,
        type=str,
        help='Output file containing the ' 'commands to train/test selected models.',
    )
    parser.add_argument(
        '--gpus_per_node',
        type=int,
    ...
    parser.add_argument(
        '--full_log', action='store_true', help='Whether save full log in a file'
    )
...
assert args.mode in ['train', 'test'], (
    'Currently we only support ' 'automatically generating training or testing scripts.'
)
...
if args.mode == 'train':
    for i, config_path in enumerate(config_paths):
        ...
        if match_obj is None:
            print(
                f'Invalid config path (no GPU num in ' f'config name):\n{config_path}'
            )
            continue
        ...
        # training commands
        script += (
            f'GPUS={gpu_num} GPUS_PER_NODE={args.gpus_per_node} '
            f'CPUS_PER_TASK={args.cpus_per_task} '
            f'./tools/slurm_train.sh $PARTITION {config_name} '
            f'{config_path} \\\n'
        )
        script += (
            f'$CHECKPOINT_DIR/{work_dir_name} --cfg-options '
        ...
        if args.full_log:
            script += f'2>&1|tee $CHECKPOINT_DIR/{work_dir_name}' f'/FULL_LOG.txt &\n'
        else:
            script += '>/dev/null &\n'
    ...
    for i, config_path in enumerate(config_paths):
        ...
        # training commands
        script += (
            f'GPUS={args.gpus} GPUS_PER_NODE={args.gpus_per_node} '
            f'CPUS_PER_TASK={args.cpus_per_task} '
        ...
        if eval_option in ['scannet_seg', 's3dis_seg']:
            ...
            script += (
                f'--format-only --eval-options jsonfile_prefix='
            ...
            script += (
                f'--eval waymo --eval-options pklfile_prefix='
