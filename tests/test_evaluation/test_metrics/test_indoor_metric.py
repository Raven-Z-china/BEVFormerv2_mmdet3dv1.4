...
class TestIndoorMetric(unittest.TestCase):
    ...
    def test_process(self, stdout):
        ...
        eval_ann_info = {
            ...
            'gt_labels_3d': np.array([2, 2, 2, 3, 4, 17, 4, 7, 2, 8, 17, 11]),
        }

        pred_instances_3d = dict()
        pred_instances_3d['scores_3d'] = torch.ones(len(eval_ann_info['gt_bboxes_3d']))
        pred_instances_3d['bboxes_3d'] = eval_ann_info['gt_bboxes_3d']
        pred_instances_3d['labels_3d'] = torch.Tensor(eval_ann_info['gt_labels_3d'])
        pred_dict = dict()
