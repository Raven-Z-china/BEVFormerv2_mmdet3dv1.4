...
def _init_evaluate_input():
    ...
    pred_instances_3d.bboxes_3d = LiDARInstance3DBoxes(
        torch.tensor([[8.7314, -1.8559, -1.5997, 0.4800, 1.2000, 1.8900, 0.0100]])
    )
...
def test_multi_modal_kitti_metric():
    ...
    kittimetric = KittiMetric(data_root + '/kitti_infos_train.pkl', metric=['mAP'])
    ...
    assert np.isclose(
        ap_dict['pred_instances_3d/KITTI/Overall_3D_AP11_easy'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances_3d/KITTI/Overall_BEV_AP11_easy'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances_3d/KITTI/Overall_2D_AP11_easy'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances/KITTI/Overall_2D_AP11_easy'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances/KITTI/Overall_2D_AP11_moderate'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances/KITTI/Overall_2D_AP11_hard'], 3.0303030303030307
    )
...
def test_kitti_metric_mAP():
    ...
    assert np.isclose(
        ap_dict['pred_instances_3d/KITTI/Overall_3D_AP11_moderate'], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict['pred_instances_3d/KITTI/Overall_3D_AP11_hard'], 3.0303030303030307
    )
