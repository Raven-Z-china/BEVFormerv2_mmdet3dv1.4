...
def test_indoor_eval():
    if not torch.cuda.is_available():
        pytest.skip()
    seg_preds = [
        np.array([0, 0, 1, 0, 0, 2, 1, 3, 1, 2, 1, 0, 2, 2, 2, 2, 1, 3, 0, 3, 3, 4, 0])
    ]
    gt_labels = [
        np.array([0, 0, 0, 4, 0, 0, 1, 1, 1, 4, 1, 1, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 4])
    ]

    label2cat = {0: 'car', 1: 'bicycle', 2: 'motorcycle', 3: 'truck', 4: 'unlabeled'}
