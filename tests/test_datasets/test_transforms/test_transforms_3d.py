...
class TestGlobalRotScaleTrans(unittest.TestCase):
    def test_globle_rotation_scale_trans(self):
        ...
        assert (
            after_rot_gt_bbox_3d.tensor[:, -1] - before_rot_gt_bbox_3d.tensor[:, -1]
        ).abs().max() < 0.79
        ...
        assert (1.06 > (after_scale_gt_bbox_3d / before_scale_gt_bbox_3d)).all()


class TestRandomFlip3D(unittest.TestCase):
    def test_random_flip3d(self):
        ori_data_info = create_data_info_after_loading()
        no_flip_transform = RandomFlip3D(flip_ratio_bev_horizontal=0.0)
        always_flip_transform = RandomFlip3D(flip_ratio_bev_horizontal=1.0)
        data_info = copy.deepcopy(ori_data_info)
        data_info = no_flip_transform(data_info)
        self.assertIn('pcd_horizontal_flip', data_info)
        assert_allclose(data_info['points'].tensor, ori_data_info['points'].tensor)

        torch.allclose(
            data_info['gt_bboxes_3d'].tensor, ori_data_info['gt_bboxes_3d'].tensor
        )
        data_info = copy.deepcopy(ori_data_info)
        data_info = always_flip_transform(data_info)
        assert_allclose(
            data_info['points'].tensor[:, 0], ori_data_info['points'].tensor[:, 0]
        )
        assert_allclose(
            data_info['points'].tensor[:, 1], -ori_data_info['points'].tensor[:, 1]
        )
        assert_allclose(
            data_info['points'].tensor[:, 2], ori_data_info['points'].tensor[:, 2]
        )

        assert_allclose(
            data_info['gt_bboxes_3d'].tensor[:, 0],
            ori_data_info['gt_bboxes_3d'].tensor[:, 0],
        )
        assert_allclose(
            data_info['gt_bboxes_3d'].tensor[:, 1],
            -ori_data_info['gt_bboxes_3d'].tensor[:, 1],
        )
        assert_allclose(
            data_info['gt_bboxes_3d'].tensor[:, 2],
...
class TestGlobalAlignment(unittest.TestCase):
    def test_global_alignment(self):
        ...
        data_info['axis_align_matrix'] = np.array(
            [
                [0.945519, 0.325568, 0.0, -5.38439],
                [-0.325568, 0.945519, 0.0, -2.87178],
                [0.0, 0.0, 1.0, -0.06435],
                [0.0, 0.0, 0.0, 1.0],
            ],
            dtype=np.float32,
...
class TestPolarMix(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        self.assertTrue(
            results['points'].shape[0] == results['pts_semantic_mask'].shape[0]
        )
