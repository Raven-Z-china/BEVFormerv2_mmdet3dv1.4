...
class TestDet3DDataSample(TestCase):
    ...
    def test_setter(self):
        ...
        gt_instances_3d_data = dict(bboxes_3d=torch.rand(4, 7), labels_3d=torch.rand(4))
        ...
        assert _equal(
            det3d_data_sample.gt_instances_3d.bboxes_3d,
            gt_instances_3d_data['bboxes_3d'],
        )
        assert _equal(
            det3d_data_sample.gt_instances_3d.labels_3d,
            gt_instances_3d_data['labels_3d'],
        )

        # test pred_instances_3d
        pred_instances_3d_data = dict(
            bboxes_3d=torch.rand(2, 7), labels_3d=torch.rand(2), scores_3d=torch.rand(2)
        )
        ...
        assert _equal(
            det3d_data_sample.pred_instances_3d.bboxes_3d,
            pred_instances_3d_data['bboxes_3d'],
        )
        assert _equal(
            det3d_data_sample.pred_instances_3d.labels_3d,
            pred_instances_3d_data['labels_3d'],
        )
        assert _equal(
            det3d_data_sample.pred_instances_3d.scores_3d,
        ...
        assert _equal(
            det3d_data_sample.pts_pred_instances_3d.bboxes_3d,
            pts_pred_instances_3d_data['bboxes_3d'],
        )
        assert _equal(
            det3d_data_sample.pts_pred_instances_3d.labels_3d,
            pts_pred_instances_3d_data['labels_3d'],
        )
        assert _equal(
            det3d_data_sample.pts_pred_instances_3d.scores_3d,
        ...
        assert _equal(
            det3d_data_sample.img_pred_instances_3d.bboxes_3d,
            img_pred_instances_3d_data['bboxes_3d'],
        )
        assert _equal(
            det3d_data_sample.img_pred_instances_3d.labels_3d,
            img_pred_instances_3d_data['labels_3d'],
        )
        assert _equal(
            det3d_data_sample.img_pred_instances_3d.scores_3d,
        ...
        assert _equal(
            det3d_data_sample.gt_pts_seg.pts_instance_mask,
            gt_pts_seg_data['pts_instance_mask'],
        )
        assert _equal(
            det3d_data_sample.gt_pts_seg.pts_semantic_mask,
        ...
        assert _equal(
            det3d_data_sample.pred_pts_seg.pts_instance_mask,
            pred_pts_seg_data['pts_instance_mask'],
        )
        assert _equal(
            det3d_data_sample.pred_pts_seg.pts_semantic_mask,
