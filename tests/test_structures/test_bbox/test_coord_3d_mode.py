...
def test_points_conversion():
    ...
    cam_points = CameraPoints(
        points_np, points_dim=7, attribute_dims=dict(color=[3, 4, 5], height=6)
    )
    ...
    lidar_point_tensor = Coord3DMode.convert_point(
        cam_points.tensor, Coord3DMode.CAM, Coord3DMode.LIDAR
    )
    ...
    depth_point_tensor = Coord3DMode.convert_point(
        cam_points.tensor, Coord3DMode.CAM, Coord3DMode.DEPTH
    )
    ...
    cam_point_tensor = Coord3DMode.convert_point(
        lidar_points.tensor, Coord3DMode.LIDAR, Coord3DMode.CAM
    )
    ...
    depth_point_tensor = Coord3DMode.convert_point(
        lidar_points.tensor, Coord3DMode.LIDAR, Coord3DMode.DEPTH
    )
    assert torch.allclose(expected_tensor, convert_depth_points.tensor, 1e-4)
    assert torch.allclose(depth_point_tensor, convert_depth_points.tensor, 1e-4)
    ...
    cam_point_tensor = Coord3DMode.convert_point(
        depth_points.tensor, Coord3DMode.DEPTH, Coord3DMode.CAM
    )
    assert torch.allclose(expected_tensor, convert_cam_points.tensor, 1e-4)
    assert torch.allclose(cam_point_tensor, convert_cam_points.tensor, 1e-4)
    ...
    depth_points_new = torch.cat(
        [depth_points.tensor[:, :3] @ rt_mat_provided.t(), depth_points.tensor[:, 3:]],
        dim=1,
    )
    ...
    lidar_point_tensor = Coord3DMode.convert_point(
        depth_points.tensor, Coord3DMode.DEPTH, Coord3DMode.LIDAR
    )
    assert torch.allclose(lidar_point_tensor, convert_lidar_points.tensor, 1e-4)
    assert torch.allclose(lidar_point_tensor, convert_lidar_points.tensor, 1e-4)
...
def test_boxes_conversion():
    ...
    convert_lidar_boxes = Coord3DMode.convert(
        cam_boxes, Coord3DMode.CAM, Coord3DMode.LIDAR
    )
    ...
    expected_tensor = torch.tensor(
        [
            ...
            [-1.7361, -26.6690, -21.8230, 1.5600, 1.4000, 3.4800, 1.6900 - np.pi / 2],
            [-1.6218, -31.3198, -8.1621, 1.7400, 1.4800, 3.7700, -2.7900 - np.pi / 2],
        ]
    )
    ...
    convert_depth_boxes = Coord3DMode.convert(
        cam_boxes, Coord3DMode.CAM, Coord3DMode.DEPTH
    )
    expected_tensor = torch.tensor(
        [
    ...
    convert_cam_boxes = Coord3DMode.convert(
        lidar_boxes, Coord3DMode.LIDAR, Coord3DMode.CAM
    )
    expected_tensor = torch.tensor(
        [
            ...
            [-21.8230, 1.7361, 26.6690, 1.5600, 1.4000, 3.4800, 1.6900 - np.pi / 2],
            [-8.1621, 1.6218, 31.3198, 1.7400, 1.4800, 3.7700, -2.7900 - np.pi / 2],
        ]
    )
    expected_tensor[:, -1:] = limit_period(expected_tensor[:, -1:], period=np.pi * 2)
    ...
    convert_depth_boxes = Coord3DMode.convert(
        lidar_boxes, Coord3DMode.LIDAR, Coord3DMode.DEPTH
    )
    expected_tensor = torch.tensor(
        [
            ...
            [-8.1621, 31.3198, -1.6218, 1.7400, 3.7700, 1.4800, 2.7900 + np.pi / 2],
        ]
    )
    expected_tensor[:, -1:] = limit_period(expected_tensor[:, -1:], period=np.pi * 2)
    assert torch.allclose(expected_tensor, convert_depth_boxes.tensor, 1e-3)
    ...
    convert_cam_boxes = Coord3DMode.convert(
        depth_boxes, Coord3DMode.DEPTH, Coord3DMode.CAM
    )
    expected_tensor = torch.tensor(
        [
            [1.7802, 1.7501, 2.5162, 1.7500, 1.6500, 3.3900, -1.4800],
    ...
    convert_lidar_boxes = Coord3DMode.convert(
        depth_boxes, Coord3DMode.DEPTH, Coord3DMode.LIDAR
    )
    expected_tensor = torch.tensor(
        [
            [2.5162, -1.7802, -1.7501, 1.7500, 3.3900, 1.6500, 1.4800 - np.pi / 2],
