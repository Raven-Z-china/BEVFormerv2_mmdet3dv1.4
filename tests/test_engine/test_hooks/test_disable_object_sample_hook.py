...
class TestDisableObjectSampleHook(TestCase):
    ...
    runner.train_dataloader.dataset.pipeline.transforms = [
        ObjectSample(
            db_sampler=dict(
                ...
                info_path='tests/data/waymo/kitti_format/waymo_dbinfos_train.pkl',  # noqa
                rate=1.0,
                prepare=dict(
    ...
    def test_is_model_wrapper_and_persistent_workers_on(self):
        self.runner.train_dataloader.dataset.pipeline.transforms[0].disabled = False
        ...
        self.assertFalse(
            self.runner.train_dataloader.dataset.pipeline.transforms[0].disabled
        )  # noqa: E501
