    ...
    def test_smoke_mono3d_head_loss(self):
        ...
        img_metas = [
            dict(
                ...
                pad_shape=[128, 128],
                trans_mat=np.array(
                    [[0.25, 0.0, 0.0], [0.0, 0.25, 0], [0.0, 0.0, 1.0]],
                    dtype=np.float32,
                ),
                affine_aug=False,
        ...
        smoke_mono3d_head = SMOKEMono3DHead(
            ...
            loss_bbox=dict(type='mmdet.L1Loss', reduction='sum', loss_weight=1 / 300),
            loss_dir=dict(
        ...
        self.assertEqual(len(ret_dict), 2, 'the length of forward feature should be 2')
        self.assertEqual(len(ret_dict[0]), 1, 'each feature should have 1 level')
        self.assertEqual(
            ret_dict[0][0].shape,
        ...
        gt_bboxes = torch.Tensor(
            [[1.0, 2.0, 20.0, 40.0], [45.0, 50.0, 80.0, 70.1], [34.0, 39.0, 65.0, 64.0]]
        )
        ...
        gt_losses = smoke_mono3d_head.loss_by_feat(
            *ret_dict, [gt_instances_3d], [gt_instances], img_metas
        )
        ...
        self.assertEqual(len(results_list), 1, 'there should be one image results')
