...
class TestAnchor3DHead(TestCase):
    def test_anchor3d_head_loss(self):
        ...
        empty_gt_losses = anchor3d_head.loss_by_feat(
            cls_scores, bbox_preds, dir_cls_preds, [gt_instances], [input_metas]
        )

        # When there is no truth, the cls loss should be nonzero but
        # there should be no box and dir loss.
        self.assertGreater(
            empty_gt_losses['loss_cls'][0], 0, 'cls loss should be non-zero'
        )
        self.assertEqual(
            empty_gt_losses['loss_bbox'][0],
            0,
            'there should be no box loss when there are no true boxes',
        )
        self.assertEqual(
            empty_gt_losses['loss_dir'][0],
            0,
        ...
        gt_losses = anchor3d_head.loss_by_feat(
            cls_scores, bbox_preds, dir_cls_preds, [gt_instances], [input_metas]
        )

        self.assertGreater(gt_losses['loss_cls'][0], 0, 'cls loss should be non-zero')
        self.assertGreater(gt_losses['loss_bbox'][0], 0, 'box loss should be non-zero')
        self.assertGreater(gt_losses['loss_dir'][0], 0, 'dir loss should be none-zero')
    ...
    def test_anchor3d_head_predict(self):
        ...
        results = anchor3d_head.predict_by_feat(
            cls_scores, bbox_preds, dir_cls_preds, input_metas
        )
