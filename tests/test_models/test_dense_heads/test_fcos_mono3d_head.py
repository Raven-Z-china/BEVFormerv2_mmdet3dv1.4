...
class TestFCOSMono3DHead(TestCase):
    def test_fcos_mono3d_head_loss(self):
        ...
        self.assertEqual(len(ret_dict), 5, 'the length of forward feature should be 5')
        self.assertEqual(len(ret_dict[0]), 5, 'each feature should have 5 levels')
        self.assertEqual(
            ret_dict[0][0].shape,
            torch.Size([1, 10, 116, 200]),
        ...
        gt_losses = fcos_mono3d_head.loss_by_feat(
        ...
        self.assertEqual(results_list_2d, None, 'there is no 2d result in fcos3d')
