...
def test_anchor_free_box_coder():
    ...
    (
        ...
        dir_class_targets,
        dir_res_targets,
    ) = box_coder.encode(gt_bboxes, gt_labels)

    expected_center_target = torch.tensor(
        [
            [2.1227, 5.7951, -0.2253],
            [11.7908, 9.0276, -0.1156],
            [23.6380, 9.6997, 0.2328],
        ]
    )
    expected_size_targets = torch.tensor(
        [[0.8368, 2.1210, 0.7736], [0.8105, 1.7683, 0.7421], [0.8789, 2.3052, 0.8000]]
    )
