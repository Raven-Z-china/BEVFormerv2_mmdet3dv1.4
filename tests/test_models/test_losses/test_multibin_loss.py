...
def test_multibin_loss():
    ...
    target = torch.tensor(
        [[1, 1, 0, 0, 2.14, 3.12, 0.68, -2.15], [1, 1, 0, 0, 3.12, 3.12, 2.34, 1.23]]
    )
    multibin_loss_cfg = dict(type='MultiBinLoss', reduction='none', loss_weight=1.0)
    ...
    assert torch.allclose(output_multibin_loss, expected_multibin_loss, atol=1e-4)
