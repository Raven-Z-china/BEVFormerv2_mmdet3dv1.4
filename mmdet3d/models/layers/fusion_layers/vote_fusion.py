...
class VoteFusion(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, max_imvote_per_pixel: int = 3) -> None:
        ...
        for i, data in enumerate(
            ...
            if bbox_num == 0:
                ...
                mask_zero = torch.zeros(
                    imvote_num - seed_num, device=seed_3d_depth.device
                ).bool()
                ...
                seed_2d_expanded = uv_origin.view(seed_num, 1, -1).expand(
                    -1, bbox_num, -1
                )
                ...
                sem_cue = sem_cue.scatter(
                    -1, bbox_expanded_cls.long(), bbox_expanded_conf
                )
                ...
                imvote = torch.cat(
                    [delta_u, delta_v, torch.zeros_like(delta_v)], dim=-1
                ).view(-1, 3)
                ...
                ray_angle /= torch.sqrt(torch.sum(ray_angle**2, -1) + EPS).unsqueeze(
                    -1
                )
                ...
                if bbox_num < self.max_imvote_per_pixel:
                    ...
                    append_zeros = torch.zeros(
                        (seed_num, append_num, 1), device=seed_2d_in_bbox.device
                    ).bool()
                    seed_2d_in_bbox = torch.cat([seed_2d_in_bbox, append_zeros], dim=1)
                    append_zeros = torch.zeros(
                        (seed_num, append_num, feature_size), device=two_cues.device
                    )
                    two_cues = torch.cat([two_cues, append_zeros], dim=1)
                    append_zeros = torch.zeros(
                        (seed_num, append_num, 1), device=two_cues.device
                    )
                ...
                mask, indices = pair_score.topk(
                    self.max_imvote_per_pixel, dim=1, largest=True, sorted=True
                )
                ...
                two_cues = (
                    two_cues.reshape(-1, feature_size).transpose(1, 0).contiguous()
                )
            ...
            uv_rescaled[:, 0] = torch.clamp(
                uv_rescaled[:, 0].round(), 0, img_shape[1] - 1
            )
            uv_rescaled[:, 1] = torch.clamp(
                uv_rescaled[:, 1].round(), 0, img_shape[0] - 1
            )
            uv_flatten = (
                uv_rescaled[:, 1].round() * img_shape[1] + uv_rescaled[:, 0].round()
            )
            uv_expanded = uv_flatten.unsqueeze(0).expand(3, -1).long()
            txt_cue = torch.gather(img_flatten, dim=-1, index=uv_expanded)
            txt_cue = (
                txt_cue.unsqueeze(1)
                .expand(-1, self.max_imvote_per_pixel, -1)
                .reshape(3, -1)
            )
