...
def apply_3d_transformation(
    pcd: Tensor, coord_type: str, img_meta: dict, reverse: bool = False
) -> Tensor:
    ...
    pcd_scale_factor = (
        img_meta['pcd_scale_factor'] if 'pcd_scale_factor' in img_meta else 1.0
    )
    ...
    pcd_horizontal_flip = (
        img_meta['pcd_horizontal_flip'] if 'pcd_horizontal_flip' in img_meta else False
    )

    pcd_vertical_flip = (
        img_meta['pcd_vertical_flip'] if 'pcd_vertical_flip' in img_meta else False
    )

    flow = (
        img_meta['transformation_3d_flow']
        if 'transformation_3d_flow' in img_meta
        else []
    )
    ...
    for op in flow:
        assert op in flow_mapping, (
            f'This 3D data ' f'transformation op ({op}) is not supported'
        )
...
def extract_2d_info(
    img_meta: dict, tensor: Tensor
) -> Tuple[int, int, int, int, Tensor, bool, Tensor]:
    ...
    img_scale_factor = (
        ...
        else tensor.new_tensor([1.0, 1.0])
    )
...
def bbox_2d_transform(img_meta: dict, bbox_2d: Tensor, ori2new: bool) -> Tensor:
...
def coord_2d_transform(img_meta: dict, coord_2d: Tensor, ori2new: bool) -> Tensor:
