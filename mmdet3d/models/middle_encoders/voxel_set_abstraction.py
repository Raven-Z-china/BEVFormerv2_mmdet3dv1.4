...
class VoxelSetAbstraction(BaseModule):
    ...
    def get_voxel_centers(
        self, coors: torch.Tensor, scale_factor: float
    ) -> torch.Tensor:
        ...
        pc_range = torch.tensor(
            self.point_cloud_range[0:3], device=voxel_centers.device
        ).float()
        voxel_centers = (voxel_centers + 0.5) * voxel_size + pc_range
        return voxel_centers

    def sample_key_points(
        self, points: List[torch.Tensor], coors: torch.Tensor
    ) -> torch.Tensor:
        ...
        for points_to_sample in src_points:
            ...
            cur_pt_idxs = furthest_point_sample(
                points_to_sample.unsqueeze(dim=0).contiguous(), self.num_keypoints
            ).long()[0]
    ...
    def forward(
        self, batch_inputs_dict: dict, feats_dict: dict, rpn_results_list: InstanceList
    ) -> dict:
        ...
        key_xyz_batch_cnt = key_xyz.new_zeros(batch_size).int().fill_(num_keypoints)
        ...
        if self.rawpoints_sa_layer is not None:
            ...
            point_features_list.append(
                pooled_features.contiguous().view(batch_size, num_keypoints, -1)
            )
        ...
        point_features = torch.cat(point_features_list, dim=-1).view(
            batch_size * num_keypoints, -1, 1
        )

        fusion_point_features = self.point_feature_fusion_layer(
            point_features.unsqueeze(dim=-1)
        ).squeeze(dim=-1)

        batch_idxs = (
            torch.arange(batch_size * num_keypoints, device=keypoints.device)
            // num_keypoints
        )  # batch indexes of each key points
