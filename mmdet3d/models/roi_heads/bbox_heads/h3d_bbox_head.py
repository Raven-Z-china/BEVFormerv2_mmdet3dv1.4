...
class H3DBboxHead(BaseModule):
    ...
    def __init__(
        ...
        assert (
            suface_matching_cfg['mlp_channels'][-1]
        ...
        conv_out_channel = (
            2
            + 3
            + bbox_coder['num_dir_bins'] * 2
            + bbox_coder['num_sizes'] * 4
            + self.num_classes
        )
    ...
    def forward(self, feats_dict: dict):
        ...
        obj_surface_center = (
            obj_surface_center.reshape(batch_size, -1, 6, 3)
            .transpose(1, 2)
            .reshape(batch_size, -1, 3)
        )
        obj_line_center = (
            obj_line_center.reshape(batch_size, -1, 12, 3)
            .transpose(1, 2)
            .reshape(batch_size, -1, 3)
        )
        ...
        line_feature = torch.cat(
            (
                line_feature.new_zeros((batch_size, 12, line_feature.shape[2])),
                line_feature,
            ),
            dim=1,
        )
    ...
    def loss_by_feat(
        ...
        **kwargs
        ...
        """
        bbox3d_optimized = self.bbox_coder.decode(feats_dict, suffix='_optimized')
        ...
        pred_obj_surface_center = (
            pred_obj_surface_center.reshape(batch_size, -1, 6, 3)
            .transpose(1, 2)
            .reshape(batch_size, -1, 3)
        )
        pred_obj_line_center = (
            pred_obj_line_center.reshape(batch_size, -1, 12, 3)
            .transpose(1, 2)
            .reshape(batch_size, -1, 3)
        )
        ...
        square_dist = self.loss_primitive_center(
            pred_surface_line_center, obj_surface_line_center
        )

        match_dist = torch.sqrt(square_dist.sum(dim=-1) + 1e-6)
        primitive_centroid_reg_loss = torch.sum(match_dist * cues_matching_label) / (
    ...
    def predict_by_feat(
        ...
        **kwargs
        ...
        # decode boxes
        obj_scores = F.softmax(feats_dict['obj_scores' + suffix], dim=-1)[..., -1]
    ...
    def get_proposal_stage_loss(
        ...
        one_hot_size_targets_expand = one_hot_size_targets.unsqueeze(-1).repeat(
            1, 1, 1, 3
        )
        size_residual_norm = (
    ...
    def get_targets(
        ...
        aggregated_points = [
            feats_dict['aggregated_points'][i] for i in range(len(batch_gt_labels_3d))
        ]

        surface_center_pred = [
            feats_dict['surface_center_pred'][i] for i in range(len(batch_gt_labels_3d))
        ]

        line_center_pred = [
            feats_dict['pred_line_center'][i] for i in range(len(batch_gt_labels_3d))
        ]
        ...
        line_center_object = [
            feats_dict['line_center_object'][i] for i in range(len(batch_gt_labels_3d))
        ]

        surface_sem_pred = [
            feats_dict['surface_sem_pred'][i] for i in range(len(batch_gt_labels_3d))
        ]

        line_sem_pred = [
            feats_dict['sem_cls_scores_line'][i] for i in range(len(batch_gt_labels_3d))
        ]
    ...
    def _get_targets_single(
        ...
        proposal_objectness_label[
            euclidean_dist1 < self.train_cfg['near_threshold']
        ] = 1
        ...
        cues_match_mask = (
            torch.sum(cues_objectness_label.view(18, num_proposals), dim=0) >= 1
        ).float()

        obj_surface_line_center = torch.cat(
            (obj_surface_center, obj_line_center), 1
        ).squeeze(0)
