...
class PartAggregationROIHead(Base3DRoIHead):
    ...
    def init_seg_head(self, seg_roi_extractor: dict, semantic_head: dict) -> None:
    ...
    def _semantic_forward_train(
        self, feats_dict: dict, voxel_dict: dict, batch_gt_instances_3d: InstanceList
    ) -> Dict:
        """Train semantic head.
    ...
    def predict(
        ...
        **kwargs
        ...
        assert self.with_semantic, 'Semantic head must be implemented' ' in PartA2.'
        ...
        results_list = self.predict_bbox(
            feats_dict, voxels_dict, batch_input_metas, rpn_results_list, self.test_cfg
        )
        return results_list
    ...
    def _bbox_forward(self, feats_dict: Dict, voxel_dict: Dict, rois: Tensor) -> Dict:
    ...
    def loss(
        ...
        **kwargs
        ...
        sample_results = self._assign_and_sample(
            rpn_results_list, batch_gt_instances_3d, batch_gt_instances_ignore
        )
        if self.with_bbox:
            feats_dict.update(semantic_results)
            bbox_results = self._bbox_forward_train(
                feats_dict, voxels_dict, sample_results
            )
            losses.update(bbox_results['loss_bbox'])

        return losses

    def _forward(self, feats_dict: dict, rpn_results_list: InstanceList) -> Tuple:
