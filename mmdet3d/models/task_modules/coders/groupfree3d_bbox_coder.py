...
class GroupFree3DBBoxCoder(PartialBinBasedBBoxCoder):
    ...
    def decode(self, bbox_out: dict, prefix: str = '') -> Tensor:
        ...
        if self.with_rot:
            ...
            dir_res = torch.gather(
                bbox_out[f'{prefix}dir_res'], 2, dir_class.unsqueeze(-1)
            )
            dir_res.squeeze_(2)
    ...
    def split_pred(
        self, cls_preds: Tensor, reg_preds: Tensor, base_xyz: Tensor, prefix: str = ''
    ) -> Dict[str, Tensor]:
        ...
        if self.size_cls_agnostic:
            ...
            results[f'{prefix}size_res'] = size_res_norm * mean_sizes.unsqueeze(
                0
            ).unsqueeze(0)
