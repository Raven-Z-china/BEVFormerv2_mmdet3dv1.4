...
class FCAF3DHead(Base3DDenseHead):
    ...
    def __init__(
        ...
        center_loss: dict = dict(type='mmdet.CrossEntropyLoss', use_sigmoid=True),
    ...
    def _make_up_block(in_channels: int, out_channels: int) -> nn.Module:
        ...
        return nn.Sequential(
            ME.MinkowskiGenerativeConvolutionTranspose(
                in_channels, out_channels, kernel_size=2, stride=2, dimension=3
            ),
            ME.MinkowskiBatchNorm(out_channels),
            ME.MinkowskiELU(),
            ME.MinkowskiConvolution(
    ...
    def forward(self, x: List[Tensor]) -> Tuple[List[Tensor], ...]:
        ...
        for i in range(len(inputs) - 1, -1, -1):
            ...
            center_pred, bbox_pred, cls_pred, point, prune_score = self._forward_single(
                out, self.scales[i]
            )
    ...
    def _forward_single(self, x: SparseTensor, scale: Scale) -> Tuple[Tensor, ...]:
    ...
    @torch.no_grad()
    def get_targets(
        self, points: Tensor, gt_bboxes: BaseInstance3DBoxes, gt_labels: Tensor
    ) -> Tuple[Tensor, ...]:
        ...
        n_levels = len(points)
        levels = torch.cat(
            [points[i].new_tensor(i).expand(len(points[i])) for i in range(len(points))]
        )
        points = torch.cat(points)
        ...
        centerness = torch.where(
            inside_box_condition, centerness, torch.ones_like(centerness) * -1
        )
        centerness = torch.where(
            level_condition, centerness, torch.ones_like(centerness) * -1
        )
        top_centerness = torch.topk(
            centerness, min(self.pts_center_threshold + 1, len(centerness)), dim=0
        ).values[-1]
