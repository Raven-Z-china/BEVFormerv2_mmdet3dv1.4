...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    def __init__(
        ...
        loss_keypoints_depth: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
    ...
    def forward_single(self, x: Tensor, batch_img_metas: List[dict]):
        ...
        if self.use_edge_fusion:
            ...
            out_cls = self.edge_fuse_cls(
                cls_feat, out_cls, edge_indices, edge_lens, feat_h, feat_w
            )
        ...
        for i in range(len(self.group_reg_dims)):
            ...
            for j, conv_reg in enumerate(self.conv_regs[i]):
                ...
                if self.use_edge_fusion and (i, j) in self.edge_fusion_inds:
                    ...
                    out_reg = getattr(self, 'edge_fuse_reg_{}_{}'.format(i, j))(
                        reg_feat, out_reg, edge_indices, edge_lens, feat_h, feat_w
                    )
    ...
    def _decode_heatmap(
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(
        ...
        pred_base_centers_2d = torch.cat(
            [topk_xs.view(-1, 1), topk_ys.view(-1, 1).float()], dim=1
        )
        preds = self.bbox_coder.decode(
            regression, batch_topk_labels, downsample_ratio, cam2imgs
        )
        ...
        pred_yaws = self.bbox_coder.decode_orientation(preds['orientations']).unsqueeze(
            -1
        )
    ...
    def get_predictions(
        ...
        w = pred_reg.shape[3]
        cam2imgs = torch.stack(
            [centers_2d.new_tensor(img_meta['cam2img']) for img_meta in batch_img_metas]
        )
        ...
        preds = self.bbox_coder.decode(
            pred_regression_pois, labels3d, downsample_ratio, cam2imgs
        )
    ...
    def get_targets(
        ...
        base_centers_2d_list, offsets_2d_list, trunc_mask_list = handle_proj_objs(
            centers_2d_list, gt_bboxes_list, batch_img_metas
        )
            ...
            for j, base_center2d in enumerate(gt_base_centers_2d):
                if trunc_masks[j]:
                    ...
                    radius = gaussian_radius(
                        [scale_box_h, scale_box_w], min_overlap=0.7
                    )
        ...
        batch_indices = [
            centers_2d_list[0].new_full((num_ctrs[i],), i) for i in range(batch_size)
        ]
        batch_indices = torch.cat(batch_indices, dim=0)
        reg_mask = torch.zeros((batch_size, max_objs), dtype=torch.bool).to(
            base_centers_2d_list[0].device
        )
    ...
    def loss_by_feat(
        ...
        loss_dir = self.loss_dir(
            preds['orientations'], target_labels['orientations_target']
        )

        # dimensions loss
        loss_dims = self.loss_dims(
            preds['dimensions'], target_labels['dimensions_target']
        )
        ...
        loss_direct_depth = self.loss_direct_depth(
            preds['direct_depth'], target_labels['depth_target'], direct_depth_weights
        )
        ...
        valid_keypoints_depth_uncertainty = preds['keypoints_depth_uncertainty'][
            depth_mask
        ]
