        ...
        if isinstance(self.bbox_assigner, list) and (not isinstance(anchors, list)):
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.assign_per_class:
                    ...
                    gt_per_cls_instance.labels_3d = gt_instance_3d.labels_3d[gt_per_cls]
                ...
                total_bbox_targets.append(
                    bbox_targets.reshape(feat_size, 1, rot_angles, anchors.size(-1))
                )
                total_bbox_weights.append(
                    bbox_weights.reshape(feat_size, 1, rot_angles, anchors.size(-1))
                )
            ...
            total_bbox_targets = torch.cat(total_bbox_targets, dim=-3).reshape(
                -1, anchors.size(-1)
            )
            total_bbox_weights = torch.cat(total_bbox_weights, dim=-3).reshape(
                -1, anchors.size(-1)
            )
    ...
    def anchor_target_single_assigner(
        ...
        if len(gt_instance_3d.bboxes_3d) > 0:
            ...
            assign_result = bbox_assigner.assign(
                pred_instance_3d, gt_instance_3d, gt_instance_ignore
            )
            sampling_result = self.bbox_sampler.sample(
                assign_result, pred_instance_3d, gt_instance_3d
            )
...
def get_direction_target(
    anchors, reg_targets, dir_offset=0, dir_limit_offset=0, num_bins=2, one_hot=True
):
