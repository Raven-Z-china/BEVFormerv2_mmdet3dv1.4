...
class SASSD(SingleStage3DDetector):
    ...
    def extract_feat(
        self, batch_inputs_dict: dict, test_mode: bool = True
        ...
        x, point_misc = self.middle_encoder(
            voxel_features, voxel_dict['coors'], batch_size, test_mode
        )
        x = self.backbone(x)
        if self.with_neck:
            x = self.neck(x)
    ...
    def loss(
        ...
        batch_gt_bboxes_3d = [
            data_sample.gt_instances_3d.bboxes_3d for data_sample in batch_data_samples
        ]
        aux_loss = self.middle_encoder.aux_loss(*point_misc, batch_gt_bboxes_3d)
