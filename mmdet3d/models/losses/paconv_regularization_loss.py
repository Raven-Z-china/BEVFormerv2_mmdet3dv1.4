...
def weight_correlation(conv: nn.Module) -> Tensor:
    ...
    flatten_kernels = (
        kernels.view(in_channels, num_kernels, out_channels)
        .permute(1, 0, 2)
        .reshape(num_kernels, -1)
    )
...
def paconv_regularization_loss(modules: List[nn.Module], reduction: str) -> Tensor:
