...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def _initialize_o3d_vis(self, show=True) -> Visualizer:
        ...
        o3d_vis.register_key_action_callback(glfw_key_space, self.space_action_callback)
    ...
    def draw_bboxes_3d(
        ...
        for i in range(len(bboxes_3d)):
            ...
            line_set.paint_uniform_color(np.array(bbox_color[i]) / 255.0)
            ...
            if self.pcd is not None and mode == 'xyz':
                ...
                self.points_colors[indices] = np.array(bbox_color[i]) / 255.0
    ...
    def set_bev_image(
        self, bev_image: Optional[np.ndarray] = None, bev_shape: int = 900
    ) -> None:
        ...
        self.ax_save.plot(
            x1, self.width / 2 - x1, ls='--', color='grey', linewidth=1, alpha=0.5
        )
        self.ax_save.plot(
            x2, x2 - self.width / 2, ls='--', color='grey', linewidth=1, alpha=0.5
        )
        self.ax_save.plot(
            self.width / 2, 0, marker='+', markersize=16, markeredgecolor='red'
        )
    ...
    def draw_bev_bboxes(
        ...
        bboxes_3d: BaseInstance3DBoxes,
        scale: int = 15,
        edge_colors: Union[str, Tuple[int], List[Union[str, Tuple[int]]]] = 'o',
        line_styles: Union[str, List[str]] = '-',
        line_widths: Union[int, float, List[Union[int, float]]] = 1,
        face_colors: Union[str, Tuple[int], List[Union[str, Tuple[int]]]] = 'none',
    ...
    def draw_proj_bboxes_3d(
        ...
        face_colors: Union[str, Tuple[int], List[Union[str, Tuple[int]]]] = 'royalblue',
    ...
    def draw_seg_mask(self, seg_mask_colors: np.ndarray) -> None:
        ...
        if hasattr(self, 'pcd'):
            offset = (
                np.array(self.pcd.points).max(0) - np.array(self.pcd.points).min(0)
            )[0] * 1.2
    ...
    def _draw_instances_3d(
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (
                ...
                img_size = (
                    img[0].shape[:2] if isinstance(img, list) else img.shape[-2:]
                )  # noqa: E501
                ...
                self.draw_proj_bboxes_3d(bboxes_3d, input_meta, edge_colors=colors)
                if vis_task == 'mono_det' and hasattr(instances, 'centers_2d'):
                    centers_2d = instances.centers_2d
                    self.draw_points(centers_2d)
    ...
    def show(
        ...
        if hasattr(self, 'o3d_vis'):
            if hasattr(self, 'view_port'):
                self.view_control.convert_from_pinhole_camera_parameters(self.view_port)
            ...
            if wait_time != -1:
                ...
                while (
                    time.time() - self.last_time < wait_time
                    and self.o3d_vis.poll_events()
                ):
                    self.o3d_vis.update_renderer()
                    self.view_port = (
                        self.view_control.convert_to_pinhole_camera_parameters()
    ...
    def space_action_callback(self, vis, action, mods):
        if action == 1:
            if self.flag_pause:
                print_log(
                    'Playback continued, press [SPACE] to pause.', logger='current'
                )
            else:
                print_log(
                    'Playback paused, press [SPACE] to continue.', logger='current'
                )
    ...
    def add_datasample(
        ...
        if (
            ...
            and 'pts_semantic_mask' in data_sample.gt_pts_seg
        ):  # noqa: E501
            keep_index = (
                data_sample.gt_pts_seg.pts_semantic_mask != ignore_index
            )  # noqa: E501
        else:
            keep_index = None
        ...
        if draw_gt and data_sample is not None:
            ...
            if 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                self._draw_pts_sem_seg(
                    data_input['points'], data_sample.gt_pts_seg, palette, keep_index
                )
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                assert 'points' in data_input
                self._draw_pts_sem_seg(
                    data_input['points'], data_sample.pred_pts_seg, palette, keep_index
                )
        ...
        if out_file is not None:
            ...
            if drawn_img is not None:
                mmcv.imwrite(
                    drawn_img[..., ::-1], out_file[:-4] + '_2d' + out_file[-4:]
                )
        else:
