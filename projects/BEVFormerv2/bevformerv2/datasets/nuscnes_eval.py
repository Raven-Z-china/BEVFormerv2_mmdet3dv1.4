...
def class_tp_curve(
    ...
    :param savepath: If given, saves the the rendering here instead of
        displaying.
...
def center_in_image(
    ...
    """Check if a box is visible inside an image without accounting for
    occlusions.

    :param box: The box to be checked.
    :param intrinsic: <float: 3, 3>. Intrinsic camera matrix.
    :param imsize: (width, height).
    :param vis_level: One of the enumerations of <BoxVisibility>. :return True
        if visibility condition is satisfied.
    """
...
def filter_eval_boxes_by_id(
    ...
    """Applies filtering to boxes.

    Distance, bike-racks and points per box.
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(
        ...
        :param eval_set: The dataset split to evaluate on, e.g. train, val or
            test.
    ...
    def update_gt(self, type_='vis', visibility='1', index=1):
        if type_ == 'vis':
            ...
            if self.visibility_test:
                """[{'description': 'visibility of whole object is between 0
                and 40%', 'token': '1', 'level': 'v0-40'}, {'description':
                'visibility of whole object is between 40 and 60%', 'token':
                '2', 'level': 'v40-60'}, {'description': 'visibility of whole
                object is between 60 and 80%', 'token': '3', 'level':
                'v60-80'}, {'description': 'visibility of whole object is
                between 80 and 100%', 'token': '4', 'level': 'v80-100'}]"""
