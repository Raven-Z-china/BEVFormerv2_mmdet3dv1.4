...
class BEVFormerV2(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        self.pts_fusion_layer = (
            MODELS.build(pts_fusion_layer) if pts_fusion_layer else None
        )
        # fcos3d head
    ...
    def forward_mono_train(self, img_feats, mono_input_dict):
        ...
        (num_gts, code_size). gt_labels_3d (list[list[Tensor]]): same as
        gt_labels centers2d (list[list[Tensor]]): 2D centers on the image with
        shape of         (num_gts, 2). depths (list[list[Tensor]]): Depth
        ground truth with shape of         (num_gts, ). attr_labels
        (list[list[Tensor]]): Attributes indices of each box. img_metas
        (list[list[dict]]): Meta information of each image, e.g.,         image
        size, scaling factor, etc. ann_idx (list[list[idx]]): indicate which
        image has mono annotation.
        """
        mono_input_dict = self.mono_format_repair(mono_input_dict)
    ...
    def forward(self,mode = 'predict', **kwargs):
        if mode == 'loss':
            return self.forward_train(**kwargs)
        else:
            kwargs.update(rescale=True)
    ...
    def forward_train(
        ...
        **mono_input_dict,
        ...
        img = img[0]
        img_metas = OrderedDict(sorted(img_metas.items()))
        img_dict = {}
    ...
    def forward_test(self, img_metas, img=None, **kwargs):
        ...
        new_prev_bev, bbox_results = self.simple_test(img_metas, img[0], prev_bev=None, **kwargs)
        ins_data = InstanceData()
    ...
    def simple_test(self, img_metas, img=None, prev_bev=None, rescale=False, **kwargs):
        ...
        img_metas = dict(sorted(img_metas.items()))
        img_dict = {}
        for ind, t in enumerate(img_metas.keys()):
            img_dict[t] = img[:, ind, ...]
        img = img_dict[0]
    ...
    def mono_format_repair(self, mono_input_dict):
        mono_input_dict['mono_ann_idx'] = [
            [idx[0] for idx in mono_input_dict['mono_ann_idx']]
        ]
        for lid, lvl in enumerate(mono_input_dict['mono_input_dict']):
            for k, v in lvl.items():
                if k != 'annotations':
                    mono_input_dict['mono_input_dict'][lid][k] = v[0]
                else:
                    for ls in range(
                        len(mono_input_dict['mono_input_dict'][lid]['annotations'])
                    ):
                        for lk, lv in mono_input_dict['mono_input_dict'][lid][
                            'annotations'
                        ][ls].items():
                            if lk != 'bbox3d':
                                mono_input_dict['mono_input_dict'][lid]['annotations'][
                                    ls
                                ][lk] = lv[0]
        mono_input_dict['mono_input_dict'] = [mono_input_dict['mono_input_dict']]
        return mono_input_dict
