...
class BEVFormerV3(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        #fusion layer
        self.pts_fusion_layer = MODELS.build(pts_fusion_layer) if pts_fusion_layer else None
        # fcos3d head
...
#        prev_bev = self.pts_fusion_layer(prev_bev, prev_img_metas)
    ...
    def mono_format_repair(self, mono_input_dict):
        mono_input_dict['mono_ann_idx'] = [[idx[0] for idx in mono_input_dict['mono_ann_idx']]]
        for lid,lvl in enumerate(mono_input_dict['mono_input_dict']):
             for k,v in lvl.items():
                 if k != 'annotations':
                     mono_input_dict['mono_input_dict'][lid][k] = v[0]
                 else:
                     for ls in range(len(mono_input_dict['mono_input_dict'][lid]['annotations'])):
                         for lk,lv in mono_input_dict['mono_input_dict'][lid]['annotations'][ls].items():
                             if lk != 'bbox3d':
                                 mono_input_dict['mono_input_dict'][lid]['annotations'][ls][lk] = lv[0]
