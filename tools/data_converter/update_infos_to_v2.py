...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    for i, ori_info_dict in enumerate(
        ...
        for k,v in ori_info_dict.items():
            if k not in {'lidar_path', 'timestamp', 'sweeps' , 'cams', 'gt_boxes', 'gt_boxes', 'gt_names', 'gt_velocity', 'num_lidar_pts', 'num_radar_pts', 'valid_flag'}:
                temp_data_info[k] = v
        ...
        for ori_sweep in ori_info_dict['sweeps']:
            ...
            temp_lidar_sweep['lidar_points'][
                'lidar2sensor'] = lidar2sensor.astype(np.float32)
