...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list['infos'])):
        ...
        for k, v in ori_info_dict.items():
            if k not in {
                'lidar_path',
                'timestamp',
                'sweeps',
        ...
        path = Path(ori_info_dict['lidar_path'])
        temp_data_info['lidar_points']['lidar_path'] = Path(*path.parts[2:])
        ...
        for cam in ori_info_dict['cams']:
            ...
            path = Path(ori_info_dict['cams'][cam]['data_path'])
            empty_img_info['img_path'] = Path(*path.parts[2:])
            empty_img_info['cam2img'] = ori_info_dict['cams'][cam]['cam_intrinsic']
            ...
            empty_img_info['sensor2lidar_rotation'] = rot
            empty_img_info['sensor2lidar_translation'] = trans
            lidar2sensor[:3, :3] = rot.T
            lidar2sensor[:3, 3:4] = -1 * np.matmul(rot.T, trans.reshape(3, 1))
            empty_img_info['lidar2cam'] = lidar2sensor.astype(np.float32)
        ...
        if 'gt_boxes' in ori_info_dict:
            ...
            for i in range(num_instances):
                ...
                empty_instance['bbox_3d'] = ori_info_dict['gt_boxes'][i, :]
        ...
        if 'pts_semantic_mask_path' in ori_info_dict:
            path = Path(ori_info_dict['pts_semantic_mask_path'])
            temp_data_info['pts_semantic_mask_path'] = Path(*path.parts[2:])
